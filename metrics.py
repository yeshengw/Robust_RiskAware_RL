...
def GetRiskMeasure(X, rm_params):
  ...
  # calculates custom risk measure of batch X
  elif rm_type == "custom":
    LQtl= torch.quantile(X, alpha)
...
def GetMetrics(X, Y, rm_params, lm_params, wass_params, rm_objective = 'maximize', problem_type = 'inner', device = torch.device('cpu')):
    ...
    # find the full inner problem gradient using inner gradient formula in paper
    ...
    # find the full outer problem gradient using outer gradient formula in paper
